				...
				<legend>Réservation Spectacle</legend>	                
					...
					<div class="collapse navbar-collapse" id="navbarSupportedContent">
						<ul class="navbar-nav mr-auto">
					      ...
					      <li class="nav-item active"><a class="nav-link" href="AnnulerSpectacle">AnnulerSpectacle<span class="sr-only">(current)</span></a></li>
					    </ul>
					...
					<fieldset>
						...
						<%if(((int)session.getAttribute("nbReservation")) >= 1){ %>							
							...
							<% for(int i=1; i<= ((int)session.getAttribute("nbReservation")); i++){ %>
								<tr>
								<td><input type="checkbox" name="reservation" value="<%= session.getAttribute("ReservationID"+i) %>"/></td>
						...
						<% if(session.getAttribute("resultDELETE")!=null){ %>
							<br>
							<p>Résultat : <strong><%= session.getAttribute("resultDELETE") %></strong></p>
