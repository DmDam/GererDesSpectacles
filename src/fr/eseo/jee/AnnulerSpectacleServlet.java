...
public class AnnulerSpectacleServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(reservation != null) {
			...
			for (String idReservation : reservation) {
				try {
					System.out.println("*1*Select booleanPaiementEffectue From Reservation Where idReservation="+idReservation+" and idClient="+idClient);
					connexionBDD1.getStnt().executeQuery("Select booleanPaiementEffectue From Reservation Where idReservation="+idReservation+" and idClient="+idClient);
					
					while(connexionBDD1.getRset().next()) {
						String resultPaiementEffectue = connexionBDD1.getRset().getString("booleanPaiementEffectue");
						System.out.println("ResultAchat : "+resultPaiementEffectue+" // "+resultPaiementEffectue.getClass());
						System.out.println("----------");
	...
	private void listReservation(HttpSession session, String idClient) {
		...
		int count = 0;
		try {
			connexionBDDReservation.getStnt().executeQuery("SELECT Reservation.idReservation, titreSpectacle, typeSpectale, ville, dateSpectacle, Reservation.booleanPaiementEffectue FROM Spectacles, Reservation WHERE Spectacles.idSpectacle=Reservation.idSpectacle AND Reservation.idClient="+idClient+" ORDER BY Reservation.booleanPaiementEffectue DESC, Spectacles.dateSpectacle, Spectacles.titreSpectacle ");
			while(connexionBDDReservation.getRset().next()) {
				...
				String resultPaiementEffectue = connexionBDDReservation.getRset().getString("Reservation.booleanPaiementEffectue");
				...
				session.setAttribute("Reservation"+count, resultTitreSpectacle+"~"+resultTypeSpectale+"~"+resultVille+"~"+resultDateSpectacle+"~"+resultPaiementEffectue);
