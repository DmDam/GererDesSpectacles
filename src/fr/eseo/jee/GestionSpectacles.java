...
public class GestionSpectacles {
	...
	public Spectacle[] trouverSpectacle(Spectacle unSpectacle) {
		...
		try {
			instanceBDD.getStnt().executeQuery("SELECT * FROM Spectacles WHERE `typeSpectale` = '"
					+ unSpectacle.getTypeSpectacle() + "' AND `ville` = '" + unSpectacle.getVilleSpectacle() + "';");
			...
			while (instanceBDD.getRset().next()) {
				...
				speclist[i] = sp;
				System.out.println("Spectacle "+i+" : "+sp+".");
				i++;
			}
		...
		try {
			instanceBDD.getStnt().executeQuery(
					"INSERT INTO `reservation`(`idReservation`, `idSpectacle`, `idClient`, `nombresPlaces`, `booleanPaiementEffectue`) VALUES ("
							+ uneReservation.getCodeReservation() + " , " + uneReservation.getCodeSpectacle() + " , "
							+ uneReservation.getCodeClient() + " , " + uneReservation.getNbPersonnes() + " , "
							+ uneReservation.getPaiement() + " ); ");
			ResultSet rset = instanceBDD.getStnt().getResultSet();
	...
	public String payerSpectacle(int codeReservation) {
		...
		try {
			instanceBDD.getStnt()
					.executeQuery("UPDATE Reservation SET booleanPaiementEffectue = TRUE WHERE idReservation = "
							+ codeReservation + " ); ");
			ResultSet rset = instanceBDD.getStnt().getResultSet();
	...
	public boolean annulerSpectacle(int codeReservation) {
		...
		try {
			instanceBDD.getStnt()
					.executeQuery("DELETE FROM reservation WHERE idReservation = " + codeReservation + " ); ");
			ResultSet rset = instanceBDD.getStnt().getResultSet();
